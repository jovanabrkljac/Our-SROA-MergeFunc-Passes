...
struct MergeFuncPass : public ModulePass {
    ...
    size_t hashFunction(Function &F){
        size_t H = hash_value(F.getFunctionType());
        H = hash_combine(H,F.size());
        for(auto &BB:F){
            H = hash_combine(H,BB.size());
            for(auto &I:BB){
                H = hash_combine(H,I.getOpcode());
                ...
                for (unsigned i=0; i< I.getNumOperands(); i++){
                    ...
                    if(isConstant(Op)){
                        H = hash_combine(H,constValue(Op));
                    }else{
                        H = hash_combine(H,Op->getType()->getTypeID());
                    }
    ...
    bool sameReturnValue(Function* F1, Function *F2){
    ...
    bool sameFunctionBody(Function *F1, Function *F2){
        ...
        auto arg1 = F1->arg_begin();
        auto arg2 = F2->arg_begin();
        for(; arg1 != F1->arg_end(); arg1++, arg2++){
            VariablesMap[&*arg1] = &*arg2;
        }
