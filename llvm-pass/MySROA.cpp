#include "llvm/IR/Function.h"
#include "llvm/IR/Instructions.h"
#include "llvm/IR/Type.h"
#include "llvm/Pass.h"
#include "llvm/Support/raw_ostream.h"
#include "llvm/IR/Verifier.h"
#include "llvm/IR/IRBuilder.h"
#include "llvm/Transforms/Utils/PromoteMemToReg.h"
#include "llvm/IR/Dominators.h"
#include <map>

using namespace llvm;

namespace {

struct MySROA : public FunctionPass {
    static char ID;
    MySROA() : FunctionPass(ID) {}

    bool runOnFunction(Function &F) override {
        bool changed = false;

        unsigned LoadsBefore = 0, StoresBefore = 0, AllocasBefore = 0, InstBefore = 0;
        for (auto &B : F) {
            for (auto &I : B) {
                InstBefore++;
                if (isa<LoadInst>(&I)) LoadsBefore++;
                if (isa<StoreInst>(&I)) StoresBefore++;
                if (isa<AllocaInst>(&I)) AllocasBefore++;
            }
        }

        errs() << "Running MySROA on function: " << F.getName() << "\n";

        unsigned Scalarized = 0, RemovedAllocas = 0;
        std::vector<Instruction*> ToErase;

        
        for (auto &B : F) {
            for (auto it = B.begin(); it != B.end();) {
                Instruction *I = &*it++;
                auto *A = dyn_cast<AllocaInst>(I);
                if (!A) {
                    continue;
                }

                auto *S = dyn_cast<StructType>(A->getAllocatedType());
                if (!S) {
                    continue;
                }

                IRBuilder<> Builder(A);
                StringRef base = A->getName();
                if (base.empty()) {
                    base = "var";
                }

                std::map<unsigned, AllocaInst*> FieldAllocas;
                for (unsigned i = 0; i < S->getNumElements(); ++i) {
                    Type *ElemTy = S->getElementType(i);
                    auto *NewA = Builder.CreateAlloca(ElemTy, nullptr,
                                                      base + "_field" + std::to_string(i));
                    FieldAllocas[i] = NewA;
                    Scalarized++;
                }


                std::vector<GetElementPtrInst*> GEPs;
                for (auto *U : A->users()) {
                    if (auto *G = dyn_cast<GetElementPtrInst>(U))
                        GEPs.push_back(G);
                }

                for (auto *GEP : GEPs) {
                    if (GEP->getNumIndices() != 2) {
                        continue;
                    }

                    auto *CI = dyn_cast<ConstantInt>(GEP->getOperand(2));
                    if (!CI) {
                        continue;
                    }

                    unsigned FieldIndex = CI->getZExtValue();
                    auto *FieldAlloca = FieldAllocas[FieldIndex];
                    if (!FieldAlloca) {
                        continue;
                    }

                    for (auto *GU : llvm::make_early_inc_range(GEP->users())) {
                        if (auto *L = dyn_cast<LoadInst>(GU)) {
                            L->setOperand(0, FieldAlloca);
                            changed = true;
                        } else if (auto *S = dyn_cast<StoreInst>(GU)) {
                            S->setOperand(1, FieldAlloca);
                            changed = true;
                        }
                    }

                    if (GEP->use_empty())
                        ToErase.push_back(GEP);
                }

                if (A->use_empty())
                    ToErase.push_back(A);
            }
        }

        for (auto *I : ToErase)
            I->eraseFromParent();

        
        std::vector<AllocaInst*> EmptyAllocas;
        for (auto &B : F) {
            for (auto &I : B) {
                if (auto *A = dyn_cast<AllocaInst>(&I)) {
                    if (A->getName().contains("_field") && A->use_empty())
                        EmptyAllocas.push_back(A);
                }
            }
        }

        for (auto *A : EmptyAllocas) {
            A->eraseFromParent();
            ++RemovedAllocas;
            changed = true;
        }

        
        DominatorTree DT(F);
        std::vector<AllocaInst*> Allocas;
        for (auto &B : F) {
            for (auto &I : B) {
                if (auto *A = dyn_cast<AllocaInst>(&I))
                    Allocas.push_back(A);
            }
        }

        if (!Allocas.empty()) {
            PromoteMemToReg(Allocas, DT);
            changed = true;
        }
        
        unsigned LoadsAfter = 0, StoresAfter = 0, AllocasAfter = 0, InstAfter = 0;
        for (auto &B : F) {
            for (auto &I : B) {
                InstAfter++;
                if (isa<LoadInst>(&I)) LoadsAfter++;
                if (isa<StoreInst>(&I)) StoresAfter++;
                if (isa<AllocaInst>(&I)) AllocasAfter++;
            }
        }

        bool Verified = !verifyFunction(F, &errs());

        errs() << " Structs decomposed: " << Scalarized
               << ", removed allocas: " << RemovedAllocas << "\n";
        errs() << " Promoted to SSA (mem2reg): " << Allocas.size() << "\n";
        errs() << " Loads: " << LoadsBefore << " -> " << LoadsAfter
               << ", Stores: " << StoresBefore << " -> " << StoresAfter
               << ", Allocas: " << AllocasBefore << " -> " << AllocasAfter << "\n";
        errs() << " Instructions: " << InstBefore << " -> " << InstAfter << "\n";
        errs() << (Verified ? " Function verified successfully\n"
                            : " Verification failed\n");
        errs() << "------------------------------------------\n";

        return changed;
    }


};

} 

char MySROA::ID = 0;
static RegisterPass<MySROA> X("mysroa", "Simple SROA");
